...
def exchange_session(*, now: float, timeout: float = 10.0, transport=http_transport) -> tuple[PaperSession, dict]:
    ...
    session = PaperSession(open_epoch=open_at, close_epoch=close_at,
                           ...
                           calendar_source=CALENDAR_SOURCE, known_at_epoch=now)
    evidence = {"date": day, "provider_open": row["open"], "provider_close": row["close"],
                ...
                "endpoint": "/v2/calendar", "host": "api.alpaca.markets", "retrieved_epoch": now,
...
def pinned_session(root: Path, *, now: float, timeout: float = 10.0, transport=http_transport):
    ...
    """
    session, evidence = exchange_session(now=now, timeout=timeout, transport=transport)
    ...
    if path.exists():
        ...
        _publish(path, {"session": facts, "pinned_at_epoch": now,
                        "scope": "Stable session facts for one date. Acquisition metadata is deliberately absent."})
        evidence = {**evidence, "session_pinned_at_epoch": now, "republished": False}
    return session, evidence
...
def read_generation(root: Path) -> tuple[Path, dict, dict]:
    ...
    input_document = json.loads((generation / "input.json").read_bytes())
    session_document = json.loads((generation / "session.json").read_bytes())
...
def feed_state(root: Path, *, now: float | None = None) -> dict:
    ...
    try:
        status = json.loads((root / "publisher-status.json").read_bytes())
...
def publish(root: Path, *, settings: dict, now: float | None = None, timeout: float = 10.0,
    ...
    try:
        ...
        _publish(root / "publisher-status.json",
                 {"status": "FAILED", "reason": type(exc).__name__ + ":" + str(exc)[:200],
...
def _publish_once(root: Path, *, settings, now, timeout, transport, clock_ns, started_ns, capture_alpaca):
    session, calendar_evidence = pinned_session(root, now=now, timeout=timeout, transport=transport)
    ...
    health = validate_capture_document(capture_document, now=now)
    ...
    provenance = {"feed": settings["feed"], "symbol": settings["symbol"],
                  ...
                  "publisher_started_ns": started_ns, "publisher_finished_ns": clock_ns(),
                  "retrieved_epoch": now, "feed_health": health}
    ...
    input_document = {"schema": "APEX_DATA_V1", "source": "ALPACA_MEASURED_CAPTURE_PUBLISHER_V1",
                      ...
                      "retrieved_utc_basis": "Publisher wall clock; per-observation receipts are measured",
    ...
    return publish_generation(root, staging, input_document, session_document,
...
def publish_generation(root: Path, staging: Path, input_document: dict, session_document: dict, *,
