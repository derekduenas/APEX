...
def timestamp_ns(text) -> int:
    ...
    datetime.fromisoformat rejects a 9-digit fraction outright, and Alpaca quotes carry one, so the fraction is
...
def from_alpaca_quotes(quotes: list, symbol: str, retrieved_utc: str, *, round_lot_shares: int = 1) -> list:
    ...
    `round_lot_shares` converts the provider's size unit to shares. It is 1 because the provider's own dated
    responses show quote sizes in the same unit and magnitude as trade sizes, which the schema states are shares
    (SPY 2026-09-14 15:30:00-15:30:30Z: quote sizes median 240 max 5800; trade sizes median 40 max 3271, and
    every quote size divisible by 40 exactly as the trades are). An unverified multiplier here silently scales
    every displayed-size cap in the evaluator, so it is a declared operator input, not a default.
    """
    out = []
    for raw in quotes:
        ...
        out.append({"kind": "quote", "symbol": symbol, "event_epoch": event_ns / 1e9,
                    ...
                    "size_conversion_basis": "OPERATOR_DECLARED_LOT_SIZE_VERIFIED_AGAINST_DATED_PROVIDER_RESPONSES",
...
def merged_document(documents: list, *, retrieved_utc: str) -> dict:
    ...
    if not documents or any(d.get("schema") != "APEX_DATA_V1" for d in documents):
        raise Refused("INVALID_DATA_DOCUMENT")
    classes = {str(d.get("source", "")).startswith("SYNTHETIC_") for d in documents}
    if len(classes) > 1:
