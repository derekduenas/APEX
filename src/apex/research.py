...
def input_class_of(document, observations):
    ...
    """
    def labels(doc):
        children = doc.get("components", [])
        values = [str(doc.get("source", "")).startswith("SYNTHETIC_")]
        if children:
            values = ([True] if values[0] else []) + [v for child in children for v in labels(child)]
        return values
    bases = {r.get("availability_basis") for r in observations + document.get("observations", []) if isinstance(r, dict)}
    synthetic = labels(document)
    has_synthetic = "SYNTHETIC_CLOCK" in bases or any(synthetic)
    has_recorded = bool(bases - {"SYNTHETIC_CLOCK"}) or (any(synthetic) and not all(synthetic))
    if has_synthetic and has_recorded:
